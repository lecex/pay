...
func config() {
	...
	if !db.DB.HasTable(&config) {
		db.DB.Exec(`
			...
			status int(11) DEFAULT 1 COMMENT '商品状态(禁用0、启用1)',
...
func alipay() {
	...
	if !db.DB.HasTable(&alipay) {
		db.DB.Exec(`
			...
			app_auth_token varchar(128) DEFAULT NULL COMMENT '第三方应用授权',
...
func wechat() {
	...
	if !db.DB.HasTable(&wechat) {
		db.DB.Exec(`
			...
			id int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID',
			app_id varchar(64) DEFAULT NULL COMMENT '应用ID',
			mch_id varchar(64) DEFAULT NULL COMMENT '商家ID',
			api_key varchar(64) DEFAULT NULL COMMENT 'API秘钥',
			sub_app_id varchar(64) DEFAULT NULL COMMENT '子应用ID',
			sub_mch_id varchar(64) DEFAULT NULL COMMENT '子商家ID',
...
func icbc() {
	...
	if !db.DB.HasTable(&icbc) {
		db.DB.Exec(`
			...
			sign_type varchar(32) DEFAULT NULL COMMENT '发送加密方式',
			return_sign_type varchar(32) DEFAULT NULL COMMENT '接受加密方式',
			mer_id varchar(64) DEFAULT NULL COMMENT '商户ID',
			sub_mer_id varchar(64) DEFAULT NULL COMMENT '子商户ID',
			fee int(11) DEFAULT 0 COMMENT '手续费单位万分之一',
			sandbox int(11) DEFAULT 0 COMMENT '沙盒模式(禁用0、启用1)',
			PRIMARY KEY (id)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
func order() {
	...
	if !db.DB.HasTable(&order) {
		db.DB.Exec(`
			...
			channel varchar(36) DEFAULT NULL COMMENT '付款方式 [支付宝、微信、工行、银联等]',
			...
			updated_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
			PRIMARY KEY (id),
			UNIQUE KEY store_id_AND_out_trade_no (store_id,out_trade_no)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8;
		`)
	}
}
